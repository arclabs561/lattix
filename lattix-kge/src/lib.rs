...
//! | [`Query2box`] | ndarray | always | Working |
//! | `TransEBurn` | burn | `burn` | Scaffolding |
//! | `MuRP` | hyp | `hyperbolic` | Planned |
//!
...
//! | Hyperbolic | Tree hierarchies | `hyp` (feature: `hyperbolic`) |
//! | Box/Region | DAGs, containment | `subsume` (feature: `boxe`) |
//!
//! ## Multi-Hop Logical Reasoning
//!
//! Standard KGE models score single triples. Reasoning models (BetaE, Query2box, GQE)
//! can answer complex logical queries like:
//! *"Scientists who were born in Germany AND won a Nobel Prize."*
//!
//! These are represented using [`LogicalQuery`] and scored using [`ReasoningModel`].
//!
//! | Model | Operator Support | Geometry |
//! |-------|------------------|----------|
//! | [`GQE`] | AND, Projection | Points |
//! | [`Query2box`] | AND, OR (DNF), Projection | Boxes |
//! | [`BetaE`] | AND, OR, NOT, Projection | Beta Distributions |
//!
...
//!
//! See `hyp` crate for Poincaré/Lorentz operations.
//!
...
//!
//! ```rust,ignore
//! use lattix_kge::{ScoringFunction, KGEmbedding};
//!
...
pub use models::{BetaE, BoxE, GQE, HSimplE, HypE, HyperFact, QualifiedFact, Query2box, StarE, TransE};
...
pub use reason::{LogicalQuery, ReasoningModel};
