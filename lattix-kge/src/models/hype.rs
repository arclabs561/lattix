                ...
                for ns in 0..config.negative_samples {
                    ...
                    if loss > 0.0 {
                        ...
                        for (&position, entity) in &hyperedge.entities {
                            ...
                            let filters_for_rel =
                                self.relation_filters.get(&hyperedge.relation).ok_or_else(
                                    || Error::RelationNotFound(hyperedge.relation.clone()),
                                )?;
                            let filter = filters_for_rel.get(position).ok_or_else(|| {
                                Error::Validation(format!(
                                    "missing relation filter: relation={}, position={}",
                                    hyperedge.relation, position
                                ))
                            })?;
                            ...
                            let filters = self
                                .relation_filters
                                .get_mut(&hyperedge.relation)
                                .ok_or_else(|| {
                                    Error::RelationNotFound(hyperedge.relation.clone())
                                })?;
                            let filter_vec = filters.get_mut(position).ok_or_else(|| {
                                Error::Validation(format!(
                                    "missing relation filter: relation={}, position={}",
                                    hyperedge.relation, position
                                ))
                            ...
                            for i in 0..self.dim {
                                filter_vec[i] += lr * entity_emb[i] * 0.5;
                            }
                        }
