...
pub struct AttH {
    ...
    /// Entity biases.
    entity_biases: HashMap<String, f32>,
    /// Relation-specific curvatures (learnable).
    relation_curvatures: HashMap<String, f32>,
    /// Relation rotation angles.
    relation_rotations: HashMap<String, Vec<f32>>,
    /// Relation reflection vectors.
    relation_reflections: HashMap<String, Array1<f32>>,
    /// Attention weights: [alpha_rot, alpha_ref] per relation.
    relation_attention: HashMap<String, (f32, f32)>,
    /// Whether trained.
    trained: bool,
}
...
impl AttH {
    ...
    fn manifold(&self, curvature: f32) -> PoincareBall<f32> {
        PoincareBall::new(curvature.abs().max(0.001))
    }

    /// Apply Givens rotation.
    fn apply_rotation(&self, v: &Array1<f32>, angles: &[f32]) -> Array1<f32> {
    ...
    fn apply_reflection(&self, v: &Array1<f32>, r: &Array1<f32>) -> Array1<f32> {
        let r_norm_sq: f32 = r.iter().map(|x| x * x).sum();
        if r_norm_sq < 1e-12 {
            return v.clone();
        }
    ...
    pub fn relation_curvature(&self, relation: &str) -> Option<f32> {
        self.relation_curvatures.get(relation).copied()
    }

    /// Get entity depth at a specific curvature.
    pub fn entity_depth(&self, entity: &str, curvature: f32) -> Option<f32> {
...
impl KGEModel for AttH {
    fn score(&self, head: &str, relation: &str, tail: &str) -> Result<f32> {
        ...
        Ok(self.atth_score(h, t, *curvature, rotation, reflection, *attention, b_h, b_t))
    }
    ...
    fn train_with_callback(
        ...
        for (i, entity) in entities.iter().enumerate() {
            ...
            let embedding: Vec<f32> = (0..self.dim)
                .map(|j| {
                    ...
                    hash.hash(&mut h);
                    j.hash(&mut h);
                    ((h.finish() as f64 / u64::MAX as f64) as f32 - 0.5) * init_scale
                })
                .collect();
        ...
        for (i, relation) in relations.iter().enumerate() {
            ...
            // Rotation angles
            let angles: Vec<f32> = (0..num_rotations)
                .map(|j| {
                    let mut h = std::collections::hash_map::DefaultHasher::new();
                    hash.hash(&mut h);
                    j.hash(&mut h);
                    ((h.finish() as f64 / u64::MAX as f64) as f32 - 0.5) * 0.2
                })
                .collect();
            self.relation_rotations.insert(relation.clone(), angles);

            // Reflection vector
            let reflection: Vec<f32> = (0..self.dim)
                .map(|j| {
                    let mut h = std::collections::hash_map::DefaultHasher::new();
                    hash.hash(&mut h);
                    (j + 1000).hash(&mut h);
        ...
        for epoch in 0..config.epochs {
            let lr = if epoch < config.epochs / 10 {
                config.learning_rate as f32 * 0.1
            } else {
