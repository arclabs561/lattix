...
pub struct MuRP {
    ...
    relation_diag: HashMap<String, Array1<f32>>,
    /// Relation translations (tangent vectors).
    relation_trans: HashMap<String, Array1<f32>>,
    /// Whether trained.
    trained: bool,
}
...
impl MuRP {
    ...
    #[inline]
    fn manifold(&self) -> PoincareBall<f32> {
    ...
    fn murp_score(
        ...
        let scaled_h: Array1<f32> = h * diag;
    ...
    pub fn entity_depth(&self, entity: &str) -> Option<f32> {
    ...
    fn init_entity_embeddings(
        ...
        for (i, item) in vocab.iter().enumerate() {
            ...
            for j in 0..self.dim {
                ...
                j.hash(&mut h);
                let val = h.finish();
                embedding.push(((val as f64 / u64::MAX as f64) as f32 - 0.5) * init_scale);
            }
    ...
    fn init_relation_params(
        ...
        vocab: &HashSet<String>,
        seed: u64,
    ) -> (HashMap<String, Array1<f32>>, HashMap<String, Array1<f32>>) {
        use std::hash::{Hash, Hasher};
        ...
        for (i, item) in vocab.iter().enumerate() {
            ...
            for j in 0..self.dim {
                ...
                j.hash(&mut h);
                let val = h.finish();
                let f = ((val as f64 / u64::MAX as f64) as f32 - 0.5);
...
impl KGEModel for MuRP {
    fn score(&self, head: &str, relation: &str, tail: &str) -> Result<f32> {
        ...
        Ok(self.murp_score(h, t, diag, trans, b_h, b_t))
    }
