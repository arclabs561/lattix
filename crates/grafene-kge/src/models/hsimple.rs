...
//!
//! ```rust,ignore
//! use grafene_kge::models::HSimplE;
//! use grafene_kge::{KGEModel, Fact, TrainingConfig};
//! use grafene_kge::models::HyperFact;
//!
        ...
        self.position_projections = (0..self.max_arity)
            .map(|pos| {
                ...
                for k in 0..proj.len() {
                    ...
                    hash.hash(&mut h);
                    k.hash(&mut h);
                    proj[k] += ((h.finish() as f64 / u64::MAX as f64) - 0.5) as f32 * 0.01;
                }
                proj
            })
            .collect();
                ...
                for fact in batch {
                    ...
                    for ns in 0..config.negative_samples {
                        ...
                        if loss > 0.0 {
                            ...
                            let r = self.relation_embeddings.get_mut(&fact.relation).unwrap();
                            for k in 0..self.dim {
                                r[k] += lr * 0.1;
                            }

                            // Update entity embeddings
                            for (_, entity) in &fact.entities {
                                let e = self.entity_embeddings.get_mut(entity).unwrap();
                                for k in 0..self.dim {
                                    e[k] += lr * 0.1;
                                }
                            }
