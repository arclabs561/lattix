...
//!
//! ```rust,ignore
//! use nexus_nn::node2vec::{SkipGram, SkipGramConfig};
//!
        ...
        for (pos, &target) in walk.iter().enumerate() {
            ...
            for ctx_pos in start..end {
                ...
                for _ in 0..neg_samples {
                    let neg_idx = rng.gen_range(0..self.neg_table.len());
...
///
/// Combines walk generation (from nexus-core) with Skip-gram training.
...
impl Node2VecTrainer {
    ...
    ///
    /// Walks should be generated using `nexus_core::algo::random_walk::Node2Vec`.
