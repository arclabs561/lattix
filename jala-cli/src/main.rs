...
use lattix_core::algo::components::{component_stats, connected_components};
use lattix_core::algo::pagerank::{pagerank, PageRankConfig};
use lattix_core::algo::random_walk::{generate_walks, RandomWalkConfig};
use lattix_core::formats::{Csv, JsonLd, NTriples, Turtle};
...
fn cmd_components(input: &PathBuf, verbose: bool, strong: bool) -> Result<()> {
    use lattix_core::algo::components::strongly_connected_components;
...
fn cmd_query(
    ...
    let triples: Vec<_> = kg
        .triples()
        .filter(|t| {
            // Use map_or for MSRV 1.75 compatibility (is_none_or requires 1.82)
            let from_match = from.as_ref().map_or(true, |f| t.subject.as_str() == f);
            let to_match = to.as_ref().map_or(true, |t_| t.object.as_str() == t_);
            let rel_match = relation
                .as_ref()
                .map_or(true, |r| t.predicate.as_str().contains(r));
