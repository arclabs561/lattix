...
impl HyperEConfig {
    ...
    pub fn with_curvature(mut self, c: f32) -> Self {
        self.curvature = c;
        self
    }
...
impl HyperE {
    ...
    pub fn score(&self, head: &str, relation: &str, tail: &str) -> Option<f32> {
    ...
    pub fn entity_embedding(&self, entity: &str) -> Option<&Array1<f32>> {
        self.entity_embeddings.get(entity)
    }

    /// Get relation embedding.
    pub fn relation_embedding(&self, relation: &str) -> Option<&Array1<f32>> {
...
pub fn train_hypere(triples: &[Triple], config: &HyperEConfig) -> Result<HyperE> {
    ...
    let mut entity_emb: HashMap<String, Array1<f32>> = entities
        .iter()
        .map(|e| {
            let emb: Array1<f32> = Array1::from_iter(
                (0..config.dim).map(|_| (rng.random::<f32>() - 0.5) * init_scale),
            );
            (e.clone(), emb)
        })
        .collect();
