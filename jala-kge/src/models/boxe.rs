            ...
            for (batch_idx, batch) in triples.chunks(config.batch_size).enumerate() {
                for triple in batch {
                    let h = self
                        .entity_embeddings
                        .get(&triple.head)
                        .ok_or_else(|| Error::EntityNotFound(triple.head.clone()))?
                        .clone();
                    let t = self
                        .entity_embeddings
                        .get(&triple.tail)
                        .ok_or_else(|| Error::EntityNotFound(triple.tail.clone()))?
                        .clone();
                    let (center, offset) = self
                        .relation_boxes
                        .get(&triple.relation)
                        .ok_or_else(|| Error::RelationNotFound(triple.relation.clone()))?
                        .clone();
                    let bump = self
                        .relation_bumps
                        .get(&triple.relation)
