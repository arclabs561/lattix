...
//!
//! ```rust,ignore
//! use lattix_kge::training::{KGETrainer, TrainingConfig};
//!
...
// Re-export Triple from lattix_core for KGE training.
// KGE literature uses (head, relation, tail); lattix_core uses (subject, predicate, object).
// They're semantically equivalent:
//   head = subject, relation = predicate, tail = object
pub use lattix_core::Triple;
        ...
        for epoch in 0..self.config.epochs {
            ...
            for batch in triples.chunks(self.config.batch_size) {
                ...
                for triple in batch {
                    ...
                    let h = entity_emb
                        .get(triple.head())
                        .ok_or_else(|| Error::EntityNotFound(triple.head().to_string()))?
                        .clone();
                    let r = relation_emb
                        .get(triple.rel())
                        .ok_or_else(|| Error::RelationNotFound(triple.rel().to_string()))?
                        .clone();
                    let t = entity_emb
                        .get(triple.tail())
                        .ok_or_else(|| Error::EntityNotFound(triple.tail().to_string()))?
                        .clone();
                    ...
                    for _ in 0..self.config.negative_samples {
                        ...
                        let t_neg = entity_emb
                            .get(neg_tail)
                            .ok_or_else(|| Error::EntityNotFound((*neg_tail).clone()))?;
...
pub mod adam {
    ...
    ///
    /// ```rust,ignore
    /// use lattix_kge::training::{Triple, TrainingConfig, adam};
    ///
