...
- `nexus::KnowledgeGraph` is currently a specific implementation (petgraph-backed).
- Implementing `sophia::graph::Graph` for `nexus::KnowledgeGraph` would allow `lattice` to plug into the wider ecosystem (serializers, reasoners, etc.).
...
**Relevance to `lattice`:**
- `nexus-core` currently implements a manual N-Triples parser.
- Switching to `rio_turtle` and `rio_xml` would provide robust, fast parsing for all RDF formats immediately.
- **Recommendation:** Replace custom parsing logic in `nexus-core` with `rio` parsers. (Done)
...
## Actionable Plan
1.  **Refactor Parsing:** Use `rio` in `nexus-core` for N-Triples/Turtle parsing. (Completed)
